...
func (fuzzer *Fuzzer) send_get_request(path string) {
...
func (fuzzer *Fuzzer) send_post_request(path string, payload string) {
	fuzzer.total_test_cases++
	co, err := udp.Dial(fmt.Sprintf("%s:%d", fuzzer.target_ip, fuzzer.target_port))
	if err != nil {
		log.Fatalf("Error dialing: %v", err)
...
func (fuzzer *Fuzzer) send_put_request(path string, payload string) {
	fuzzer.total_test_cases++
	co, err := udp.Dial(fmt.Sprintf("%s:%d", fuzzer.target_ip, fuzzer.target_port))
	if err != nil {
		log.Fatalf("Error dialing: %v", err)
...
func (fuzzer *Fuzzer) send_delete_request(path string) {
	fuzzer.total_test_cases++
	co, err := udp.Dial(fmt.Sprintf("%s:%d", fuzzer.target_ip, fuzzer.target_port))
	if err != nil {
		log.Fatalf("Error dialing: %v", err)
...
func (fuzzer *Fuzzer) run_fuzzer(path string, payload string) {
	// send a GET request
...
func main() {
	...
	fuzzer := Fuzzer{target_ip: "127.0.0.1", target_port: 5683, total_test_cases: 0}
	fuzzer.get_paths()
	payload := "Hello World"
